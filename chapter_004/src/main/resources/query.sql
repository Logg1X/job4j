CREATE_TABLE_TRACKER=CREATE TABLE IF NOT EXISTS tracker(id serial PRIMARY KEY,name VARCHAR (200),description TEXT,create_date TIMESTAMP WITHOUT TIME ZONE,update_date TIMESTAMP WITHOUT TIME ZONE);
CREATE_TABLE_COMMENTS=CREATE TABLE IF NOT EXISTS comments(id serial PRIMARY KEY,message TEXT,create_date TIMESTAMP WITHOUT TIME ZONE,id_item INTEGER REFERENCES tracker(id));
ADD_ITEM=INSERT INTO tracker(name,description,create_date,update_date)VALUES (?, ?, ?, ?);
